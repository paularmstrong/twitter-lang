...
URL
  ...
  / domain:$(SubDomain? DomainChar+ ".") tld:TLD rest:$(Port? Path*) &(Space / End)
    {
      return validGenericTLD.test(tld) ||
        validSpecialCasedCCTLD.test(tld) ||
        (validCCTLD.test(tld) && /\.co\.$/.test(domain)) ||
        (rest && validCCTLD.test(tld)) ?
...
DomainChar
  = "xn--"i [0-9a-z]i+
  / $(ValidDomainChar ("-"? ValidDomainChar)+)
  / ValidDomainChar
...
SubDomain
  ...
  / $(SubDomainChar+ "." SubDomain) &Domain
